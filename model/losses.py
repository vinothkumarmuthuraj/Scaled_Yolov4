...
class Yolov3ClassificationLoss():
    ...
    def focal(args, grid_index):
        def loss(y_true, y_pred):
            ...
            y_true = y_true[..., 5:]
            y_pred = y_pred[..., 5:]
            y_pred_sig = tf.math.sigmoid(y_pred)
#             alpha_weight = tf.where(y_true != 0, args.focal_alpha, 1.-args.focal_alpha)
#             focal_weight = tf.where(y_true != 0, 1. - y_pred, y_pred)
            
            alpha_weight = tf.where(y_true > 0.90, args.focal_alpha, 1.-args.focal_alpha)
            focal_weight = tf.where(y_true > 0.90, 1.-y_pred_sig, y_pred_sig)
...
def yolov3_loss(args, grid_index):
    ...
    def loss(y_true, y_pred):
        ...
        box_reg_loss1 = box_reg_losss_weight*box_reg_loss
        obj_loss1 = obj_losss_weight*obj_loss
        cls_loss1 = cls_losss_weight*cls_loss
        
        return tf.reduce_sum(box_reg_loss1,axis=0), tf.reduce_sum(obj_loss1,axis=0), tf.reduce_sum(cls_loss1,axis=0)
